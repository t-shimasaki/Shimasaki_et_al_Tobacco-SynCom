...
```{r paths}
data_dir    <- "/Users/tomohisa/Desktop/Shimasaki_et_al_2026/02_metabolites_analysis/input/"
fig_dir     <- "/Users/tomohisa/Desktop/Shimasaki_et_al_2026/02_metabolites_analysis/output/"
```
...
```{r}
Art    <-  melt(
  individual[individual$Time > 2 & individual$Time < 10,
...
tab_mSynCom$variable    <-  factor(tab_mSynCom$variable,
                                  levels = c("Control", "NtRootA2", "NtRootA2 (ΔndhL)", "NtRootA2 (VC)",
...
p3 <- ggplot(tab_mSynCom, aes(x = Time, y = value)) +
  ...
  theme(
    strip.placement = "outside",
    strip.background = element_blank(),
    strip.text.y.left = element_text(size = 5, angle = 90),
...
## Plot data
```{r}
p4 <- ggplot(tab_SynCom[tab_SynCom$time>0.75&tab_SynCom$time<1.25,], 
